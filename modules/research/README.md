# Module 1 — Research (`crypto_research`)

Research module of `crypto-trading-system`: historical market data from
the Bitunix public API, validation/normalisation, Parquet storage, local
indicator computation, a deterministic rule-based **Market Regime
Detector**, and reproducible research reports.

This module does **not** trade: no order execution, no paper trading, no
AI/ML decisions, no Telegram/Supabase, no portfolio or risk-for-execution
logic. Regime labels are *not* buy/sell signals — the future Strategy
module (see `modules/strategy/README.md`) consumes this module's outputs
through data contracts, never through its internals.

Repository overview lives in the root `README.md`.
...
│   └── utils/               # logging
├── tests/                   # pytest suite (data / indicators / regime / CLI / config)
├── config/default.yaml      # module configuration (assets, timeframes, thresholds)
├── data/                    # module-owned research datasets (raw/processed/metadata)
├── reports/                 # generated markdown reports
└── pyproject.toml           # module packaging (package: crypto_research)
```
...
CLI entry points: `python -m crypto_research ...` (or the installed
`research` script).
...
```bash
python -m crypto_research download --symbol BTCUSDT --timeframe 4h --start 2022-04-17
python -m crypto_research download --symbol BTCUSDT --timeframe 4h --market spot   # kline/history (deep)
python -m crypto_research download --symbol BTCUSDT --timeframe 4h --end 2024-06-30
```
...
```bash
python -m crypto_research download   --symbol BTCUSDT --timeframe 4h --start 2022-04-17
python -m crypto_research indicators --symbol BTCUSDT --timeframe 4h [--market futures] [--allow-non-ready]
python -m crypto_research regime     --symbol BTCUSDT --timeframe 4h [--market futures] [--allow-non-ready]
python -m crypto_research report     --symbol BTCUSDT --timeframe 4h [--market futures] [--allow-non-ready]
python -m crypto_research config     # print resolved configuration
```
